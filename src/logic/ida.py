...
class IDAStar:
    def __init__(self, puzzle):
        """Intializes the IDA* algorithm heuristics, from the file with the patterns and their weights.
        """      
        self.puzzle = puzzle
        try:
            with open("patterns.dat", "rb") as file:
                ...
                if sum(len(group) for group in self.groups) != self.puzzle.size ** 2 - 1:
                    print("Cannot perform IDA* to solve the Puzzle, because the additive pattern database does not match with this Puzzle.")
                    self.groups = None
                else:
                    print("To solve the Puzzle, IDA* algorithm will use the additive pattern database for heuristics, with the groupings of "
    ...
    def start(self):
        """If the groups have been assigned, initializes the starting position of the algorithm, and the required data structures. 
        Then, until the Puzzle is solved, the algorithm will iterate with the new bound values found.
        ...
        while True:
            ...
            if t is True:
                print(f"The algorithm found the path in {round(time() - start, 2)} seconds.")
                return directions
            bound = t
            
    def search(self, path: list, directions: list, g: int, bound: int):
        """Recursively finds the optimal path to the goal state using the IDA* (Iterative Deepening A*) algorithm. 
    The algorithm takes the last puzzle in the path and calculates its 'f' value (moves + heuristic). 
    If this value exceeds the given bound, the algorithm backtracks and returns the minimum possible bound from 
    this path. If all the directions exceed the bound, the bound is updated and the search is repeated with the new bound. 
    If the goal state is reached, the recursion terminates and returns True. 

        Args:
        path (list): A list of Puzzles representing the current path.
        directions (list): A list of directions taken to reach the current state from the previous states.
    ...
    def heuristic(self, puzzle: Puzzle):
        ...
        for g in range(len(self.groups)):
            hashed_puzzle = puzzle.hash(self.groups[g])
            self.patterns[g].get(hashed_puzzle)
            bound += self.patterns[g].get(hashed_puzzle)
        return bound